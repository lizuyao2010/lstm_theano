...
def test(testdata,f_log,epi):
    ...
    for item in testdata:
      scoretable=[]
      qcode=q_encoder(encode_indexs(item['qcode'],volsize,max_sen_len))
      for key in item:
...
if pretrained_model:
  ...
  W_s = theano.shared(model['W_s'],name='W_s')
...
X_emb=T.sum(theano.dot(X,Een).dimshuffle(1,0)*W_s,axis=1)
Y_emb=T.sum(Ren[Y],axis=0)
Z_emb=T.sum(Ren[Z],axis=0)


X_embs=T.sum(theano.dot(Xs,Een).dimshuffle(0,2,1)*W_s,axis=2)
...
gEen, gRen, gW_s = T.grad(Cost, params)
# compute the gradients with respect to the model parameters
gparams = [gEen, gRen,gW_s]# compute list of weights updates
...
aW_s = theano.shared(np.zeros((max_sen_len),dtype=dtype),name='aW_s')
_accugrads=[aEen,aRen,aW_s]
