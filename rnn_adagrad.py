...
def encoder(wordt, htm1, 
            Een, Wxhen, When, bhen):
    # xt = theano.dot(wordt, Een)
    xt = Een[wordt]
    ht = sigma(theano.dot(xt, Wxhen) + theano.dot(htm1, When) + bhen)
    return ht
...
if pretrained_model:
  ...
  When = theano.shared(model['When'], name='When')
  bhen = theano.shared(model['bhen'], name='bhen')
  Wxhen = theano.shared(model['Wxhen'], name='Wxhen')
  ...
  When = theano.shared(sample_weights(nen,nen), name='When')
  bhen = theano.shared(inputbias*np.ones((nen,),dtype=dtype), name='bhen')
  Wxhen = theano.shared(sample_weights(nen,nen), name='Wxhen')
...
theano.scan(fn=encoder,\
            ...
            outputs_info=[dict(initial=h0en,taps=[-1])],\
            non_sequences=[Een, Wxhen, When, bhen])
...
l2 = T.sum(Een**2) + T.sum(Ren**2) + T.sum(Wxhen**2) + T.sum(When**2) + T.sum(bhen**2)
...
mWxhen = theano.shared(np.zeros((nen,nen),dtype=dtype), name='mWxhen')
mWhen = theano.shared(np.zeros((nen,nen),dtype=dtype), name='mWhen')
mbhen = theano.shared(np.zeros((nen,),dtype=dtype), name='mbhen')


mparams = [mEen, mRen, mWxhen, mWhen, mbhen]


#TODO: take grads
params = [Een, Ren, Wxhen, When, bhen]

gEen, gRen, gWxhen, gWhen, gbhen \
= T.grad(Cost, params)

gparams = [gEen, gRen, gWxhen, gWhen, gbhen]
...
for epi in xrange(max_epoch):
    ...
    model['When'] = When.get_value()
    model['bhen'] = bhen.get_value()
    model['Wxhen'] = Wxhen.get_value()
