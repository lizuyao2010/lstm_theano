...
theano.scan(fn=encoder,\
            ...
            non_sequences=[Een, Wxien, Whien, bien, Wxfen, Whfen, bfen,\
                           Wxcen, Whcen, bcen, Wxoen, Whoen, boen])
Y_emb=act(T.sum(theano.dot(Y,Ren),axis=0))
Z_emb=act(T.sum(theano.dot(Z,Ren),axis=0))
...
vRen = theano.shared(np.zeros((relsize,nen),dtype=dtype),name='vRen')
...
vparams = [vEen, vRen, vWxien, vWhien, vbien, vWxfen, vWhfen, vbfen,
...
learn_model_fn = theano.function(inputs = [X, Y, Z],
                                 outputs = Cost,
                                 updates = updates)
...
for epi in xrange(max_epoch):
    ...
    for offset in range(0,trainsize,batchsize):
        ...
        train_cost=learn_model_fn(batch[0],batch[1],batch[2])
        err+=train_cost
